...
class MarketProcessor(BaseProcessor):
    ...
    def _create_new_rows(self, targets, historical_data, sector):
        ...
        try:
            ...
            total_quarters = len(
                statements_quarters
            )  # Store the total number of quarters
            ...
            for i, (_, row) in enumerate(statements_quarters.iterrows()):
                ...
                for ticker in tickers:
                    if ticker:
                        ...
                        one_row = self._create_one_row(
                            targets, company_name, quarter, ticker, historical_data
                        )
                ...
                if (
                    remaining == 0
                    or remaining % self.config.scraping["batch_size"] == 0
    ...
    def _create_one_row(self, targets, company_name, quarter, ticker, historical_data):
        ...
        try:
            ...
            one_row_description = self.config.domain["tipos_acoes"].get(
                tick_type, "Tipo de Ação Desconhecido"
            )
    ...
    def main(self, thread=True):
        ...
        try:
            ...
            historical_data = self.load_data(
                table_name=self.config.historical_data,
                db_filepath=self.config.databases["raw"]["filepath"],
            )
            ...
            targets = pd.merge(
                statements_data,
                companies_data,
                on=self.config.historical_columns_both,
                how="left",
            )
            ...
            historical_data_rows = self._create_new_rows(
                targets, historical_data, sector
            )
            new_rows = pd.DataFrame(historical_data_rows)

            result = pd.concat([targets, new_rows], ignore_index=True).drop_duplicates(
                keep="last"
            )
            result = result.sort_values(
                by=self.config.statements_order,
                ascending=[True] * len(self.config.statements_order),
            )
