...
class CorporateEventsProcessor(BaseProcessor):
    ...
    def process_batch(self, sub_batch, progress):
        ...
        try:
            ...
            for i, ticker in enumerate(sub_batch):
                urls = self.specific_method(months, ticker)
                rows.extend(urls)
    ...
    def specific_method(self, months, ticker):
        '''
    ...
    def main_original(self, thread=True):
        ...
        try:
            ...
            fast_debug_tickers = ['UQMU', 'PTGU', 'TZRD', 'QVQP', 'NNPY', 'GRDR', 'BODY', 'AACL', 'AYOS', 'AMCT', 'ATBC', 'ABAP', 'ABPR', 'ABCC', 'ABMX', 'ABGD', 'SIVR', 'ABRL', 'ACEC', 'FCEF', 'ACEE', 'ACHE', 'ACOV', 'ACQO', 'ACPE', 'QUAT', 'ADMA', 'ADDI', 'ADAG', 'ADGE', 'AECY', 'AEGP', ]
            ticker_urls = {}
                            ...
                            if 1 == 1:
                                ...
                                for table in nested_tables:
                                    try:
                                        ...
                                        df = pd.read_html(StringIO(cleaned_table))[0]  # Convert the table to a DataFrame
                                        table_id = str(df.iloc[0, 0])
                                        if 'Nome da Ação' in table_id:
                                            ticker_info = table_id.split(':')[-1].strip()  # Extracts 'ticker_type (ticker)' part
                                            ticker_type, ticker_code = ticker_info.split()  # Splits 'ticker_type' and '(ticker)'
                                            ...
                                            df['date'] = pd.to_datetime(df['date'].apply(lambda x: f"{year.zfill(4)}-{month.zfill(2)}-{x.zfill(2)}"), format='%Y-%m-%d', errors='coerce')
